...
namespace flare::json2pb {
    proto_to_json_options::proto_to_json_options()
            : enum_option(OUTPUT_ENUM_BY_NAME), pretty_json(false), enable_protobuf_map(true), bytes_to_base64(true),
              jsonify_empty_array(false), always_print_primitive_fields(false) {
    }

    class PbToJsonConverter {
    public:
        explicit PbToJsonConverter(const proto_to_json_options &opt) : _option(opt) {}
...
#undef CASE_FIELD_TYPE
                ...
                if (field->is_repeated()) {
                    ...
                    for (int index = 0; index < field_size; ++index) {
                        ...
                        if (field->type() == google::protobuf::FieldDescriptor::TYPE_BYTES
                            ...
                            flare::base::base64_encode(value, &value_decoded);
    ...
    bool ProtoMessageToJsonStream(const google::protobuf::Message &message,
        ...
        if (options.pretty_json) {
            RAPIDJSON_NAMESPACE::PrettyWriter<OutputStream> writer(os);
            succ = converter.Convert(message, writer);
        } else {
            RAPIDJSON_NAMESPACE::OptimizedWriter<OutputStream> writer(os);
            succ = converter.Convert(message, writer);
        }
        if (!succ && error) {
            error->clear();
    ...
    bool proto_message_to_json(const google::protobuf::Message &message,
        ...
        return proto_message_to_json(message, json, proto_to_json_options(), error);
    }

    bool proto_message_to_json(const google::protobuf::Message &message,
                            google::protobuf::io::ZeroCopyOutputStream *stream,
                            const proto_to_json_options &options, std::string *error) {
    ...
    bool proto_message_to_json(const google::protobuf::Message &message,
                            google::protobuf::io::ZeroCopyOutputStream *stream,
                            std::string *error) {
        return proto_message_to_json(message, stream, proto_to_json_options(), error);
    }
} // namespace flare::json2pb
