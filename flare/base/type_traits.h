...
namespace flare {
    ...
    struct is_initializer_list_impl {
        static std::false_type is_initializer_list_dispatch(...);  // default: No
        template<typename T>
        static std::true_type is_initializer_list_dispatch(std::initializer_list<T> *);
    };

    template<typename T>
    struct is_initializer_list
            : decltype(is_initializer_list_impl::is_initializer_list_dispatch(static_cast<T *>(nullptr))) {
    };
